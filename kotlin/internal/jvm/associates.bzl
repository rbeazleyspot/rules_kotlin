...
def _collect_associates(ctx, toolchains, associate):
    ...
    1. When the associate provides an associates_abi_jar (generated by ExperimentalAssociatesAbiGen plugin),
        that jar is used directly. It contains ABI classes with internal visibility preserved.
    2. When `experimental_remove_private_classes_in_abi_jars` is enabled and the tag override has not been provided, only the
    ...
    # Prefer the associates ABI jar when available — it's an ABI jar that preserves internal visibility
    associates_abi_jar = associate[_KtJvmInfo].associates_abi_jar if _KtJvmInfo in associate and hasattr(associate[_KtJvmInfo], "associates_abi_jar") else None
    ...
    elif (not "kt_remove_private_classes_in_abi_plugin_incompatible" in ctx.attr.tags and
