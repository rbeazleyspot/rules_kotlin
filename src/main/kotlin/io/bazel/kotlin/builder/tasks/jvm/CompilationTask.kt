...
/**
 * Produce the associates ABI jar (with internal visibility preserved).
 */
internal fun JvmCompilationTask.createAssociatesAbiJar() =
  JarCreator(
    path = Paths.get(outputs.associatesAbiJar),
    normalize = true,
    verbose = false,
  ).also {
    it.addDirectory(Paths.get(directories.associatesAbiClasses))
    it.addDirectory(Paths.get(directories.generatedClasses))
    it.setJarOwner(info.label, info.bazelRuleKind)
    it.execute()
  }
